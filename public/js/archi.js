...
function create_house()
{
    ...
    document.getElementById('show-plane').innerHTML="显示平面图";
    document.getElementById('export').style.opacity=0;
    document.getElementById('exp-scene').innerHTML='<span class="glyphicon glyphicon-download-alt">';
        ...
        for(var i=0;i<archi_window.length;i++)
        {
            ...
            for(var j=0; j<aux_polyline.length; j++)
            {
                if(archi_window[i].id == aux_polyline[j].id)
                {
                    ...
                    cubeobj.add(mesh_wall_down);                            
                    cubeobj.add(mesh_wall_up);
...
///////////////////////////////////////////////////////////////////////////////////
var amb_light, dir_light, spot_light;
function initLight(){
    //ambient
    amb_light= new THREE.AmbientLight(0x969696, 0.8);
    scene.add(amb_light);
////////////////////////////////////////////////////////////////////////

    var spot_light = new THREE.SpotLight( 0xffffff, 0.5);
    spot_light.position.set( 0, 2000, 0 );
    spot_light.castShadow = true;

    spot_light.shadowCameraNear = 200;
    spot_light.shadowCameraFar = camera.far;
    spot_light.shadowCameraFov = 50;

    spot_light.shadowBias = -0.00022;
...
///////////////////////////////////////////////////////
    ...
    dir_light = new THREE.DirectionalLight( 0xdfebff, 0.8 );
    dir_light.position.set( 500, 1000, 500 );
    dir_light.position.multiplyScalar( 1.3 );
...
function change_amblight(i){
    var amb=document.getElementById("light0").value;

    if(i==0){
        if(amb=="0"){
            scene.add(amb_light);
            document.getElementById("light0").value = amb_light.intensity.toFixed(1);
        }
        else{
            scene.remove(amb_light);
            document.getElementById("light0").value = 0;
        }
    }
    else if(i==1){
        if(amb_light.intensity>0.1){
...
function change_light(i){
    switch(i){
        case 0: if(amb_light.intensity>0.1) amb_light.intensity-=0.1; break;
        case 1: amb_light.intensity+=0.1; break;
        case 2: if(dir_light.intensity>0.1) dir_light.intensity-=0.1; break;
        case 3: dir_light.intensity+=0.1; break;
        // case 4: if(spot_light.intensity>0.1) spot_light.intensity-=0.1; break;
        // case 5: spot_light.intensity+=0.1; break;

    }
    document.getElementById("light0").value = amb_light.intensity.toFixed(1);
    document.getElementById("light1").value = dir_light.intensity.toFixed(1);
    //document.getElementById("light2").value = spot_light.intensity.toFixed(1);
}

function shadow(i){
    if(i==0){
        dir_light.castShadow = false;
    }
    else if(i==1){
        dir_light.castShadow = true
    }
...
function exportToObj(bt) {
    ...
    if(exp.style.opacity==0){
        ...
        var result = exporter.parse( cubeobj );
        exp.style.opacity=1;
        ...
        exp.style.opacity=0;
        bt.innerHTML='<span class="glyphicon glyphicon-download-alt">';
    }
...
function floor_texture_size(i){
    ...
    material_floor.map.repeat.set( uvscale_floor, uvscale_floor );
}
...
function wall_texture_size(i){
    ...
    material_wall.map.repeat.set( uvscale_wall, uvscale_wall);
}
...
function add_furniture(i, furname, furname_cn){
    var loader = new THREE.JSONLoader();
    var filename="models/";

    switch(i){
        case 0: filename+="test/";break;
        case 1: filename+="common/";break;
        case 2: filename+="livingroom/";break;
        case 3: filename+="bedroom/";break;
        case 4: filename+="studyroom/";break;
        case 5: filename+="kitchen/";break;
        case 6: filename+="toilet/";break;
    }
...
function del_all_fur(){
    for(i=0;i<objects.length;i++){
        curhouse.remove(objects[i]);
    }
...
function create_sample(){
    ...
    loader.load('models/floor1_fur4.js', function ( geometry, materials ) {
        var material = new THREE.MultiMaterial( materials );
        //material.side = THREE.DoubleSide;
        mesh_fur1 = new THREE.Mesh( geometry, material );

        mesh_fur1.traverse(function(child) {
            if (child instanceof THREE.Mesh) {
                child.material.side = THREE.DoubleSide;
            }
        });
        curhouse.add(mesh_fur1);
...
function create_sample1(){
    ...
    loader.load('models/eg2.js', function ( geometry, materials ) {
        var material = new THREE.MultiMaterial( materials );
        //material.side = THREE.DoubleSide;
        mesh_fur2 = new THREE.Mesh( geometry, material );

        mesh_fur2.traverse(function(child) {
            if (child instanceof THREE.Mesh) {
                child.material.side = THREE.DoubleSide;
            }
        });
        curhouse.add(mesh_fur2);
...
function del_sample(){
    curhouse.remove(mesh_fur1);
    curhouse.remove(mesh_fur2);
}
