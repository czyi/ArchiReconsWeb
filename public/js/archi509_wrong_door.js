...
function create_house(hstr)
{
   // document.getElementById("ptxt1").innerHTML=hstr;
    ...
    // var curdoor = new THREE.Object3D();
    // //var curdoor1= new THREE.Object3D();
     var loader = new THREE.OBJLoader();
    // loader.load('models/door.obj', function(obj) {
    //     //双面绘制
    ...
    //     mesh_predoor = obj; //储存到全局变量中
    //     curhouse.add(obj);
    //     curdoor.add(mesh_predoor);
        
    //     //curdoor1.add(mesh_predoor);
    // });

    // curdoor.position.x=200;
    // curhouse.add(curdoor);
    ...
    for(var k=0;k<5;k++){
        ...
        //     var mesh_new = new THREE.Mesh(obj.child,obj.material);; //储存到全局变量中
        //     mesh_new.position.z=100*k;
        //     curhouse.add(mesh_new);
        // });

        //var newob = new THREE.Mesh(mesh,material);
        newob=mesh;
        newob.position.set(0,0,100*k);
        //scene.add(mesh);
        curhouse.add(newob);     
    }
    ...
    var mesh_door = new Array();
    var newdoor = new Array(20);
    if(archi_door)
    {//create door          
        for(var i=0;i<3;i++)
        {
            ...
            for(var j=0; j<aux_polyline.length; j++)
            {
                if(archi_door[i].id == aux_polyline[j].id)
                {
                    ...
                    //var mesh_door = new THREE.Mesh(door,material_door);
                    var mesh_wall_up= new THREE.Mesh(wall_up,material_wall);
                    ...
                    // mesh_door.position.y=Math.round(height/2)+Math.round(altitude);
                    ...
                    // mesh_door[i]=curdoor;
                    // mesh_door[i].rotateY(-angle);
                    // mesh_door[i].position.x=aux_polyline[j].point[0].x/2+aux_polyline[j].point[1].x/2-floor_x/2;
                    // mesh_door[i].position.y=0;
                    // mesh_door[i].position.z=aux_polyline[j].point[0].z/2+aux_polyline[j].point[1].z/2-floor_z/2;
                    // curhouse.add(mesh_door[i]);
                    ...
                    //     });
                    //     obj.rotateY(-angle);
                    //     obj.position.x=aux_polyline[j].point[0].x/2+aux_polyline[j].point[1].x/2-floor_x/2;
                    //     obj.position.y=0;
                    //     obj.position.z=aux_polyline[j].point[0].z/2+aux_polyline[j].point[1].z/2-floor_z/2;
                    //     curhouse.add(obj);
                    // });
