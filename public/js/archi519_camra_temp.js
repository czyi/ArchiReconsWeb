...
var texture_wall, texture_floor;
...
function left_rotate(){
    ...
    // var r=Math.sqrt(pre_x*pre_x+pre_z*pre_z);
    // var r_cos=Math.acos(pre_z/r);


    // if(pre_x<0) r_cos+=Math.PI;

    // document.getElementById("ptxt1").innerHTML="angle is "+r_cos;



    // camera.position.x = r*Math.sin(r_cos+Math.PI/6);
    // camera.position.z = r*Math.cos(r_cos+Math.PI/6);
...
function change_color(element){
    ...
    //material_door.color="0x"+color.substring(1);

    //material_door.color=new THREE.Color(1,0,0);
   // material_door.color=new THREE.Color(0x8B4500);

    //material_door = new THREE.MeshLambertMaterial({color:"0x"+color.substring(1), side: THREE.DoubleSide});//棕色0x8B4500
    //0x8B4500
    //create_house();
