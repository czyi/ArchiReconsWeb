...
var premesh_win;
var prewin_width=104, prewin_height=160, prewin_thick=23;
var predoor_width=125, predoor_height=220, predoor_thick=23;
...
function create_house(hstr)
{
   // document.getElementById("ptxt1").innerHTML=hstr;
    ...
    // var temp_win = new THREE.Mesh(win_pregeo, win_premat);
    // curhouse.add(temp_win);
        ...
        for(var i=0;i<archi_window.length;i++)
        {
            ...
            for(var j=0; j<aux_polyline.length; j++)
            {
                if(archi_window[i].id == aux_polyline[j].id)
                {
                    ...
                    // var win=win_pregeo;
                    // win.scale(0.99, 1, 1);
                    var mesh_win = new THREE.Mesh(win_pregeo, win_premat);
                    //var mesh_win=premesh_win.clone();
                    document.getElementById("ptxt1").innerHTML="scale is " + (width-thickness)/prewin_width + ", " + height/prewin_height + ", " + thickness/prewin_thick;
                    //mesh_win.geometry.scale((width-thickness)/prewin_width, height/prewin_height, 1);
                    var win_scale=new THREE.Object3D();
                    win_scale.add(mesh_win);
                    win_scale.scale((width-thickness)/prewin_width, height/prewin_height, 1);
                    //mesh_win.geometry.scale((width-thickness)/prewin_width, altitude/prewin_height, thickness/prewin_thick);
