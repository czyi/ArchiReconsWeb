           //创建3D模型
            ...
            {
                xmlhttp2=null;
                if (window.XMLHttpRequest) xmlhttp2=new XMLHttpRequest(); // code for Firefox, Opera, IE7, etc.
                else if (window.ActiveXObject) xmlhttp2=new ActiveXObject("Microsoft.XMLHTTP"); // code for IE6, IE5

                if (xmlhttp2!=null){
                    xmlhttp2.onreadystatechange=house_info;
                    xmlhttp2.open("GET",'/init',true);
                    xmlhttp2.send(null);
                }else{
            ...
            function house_info()
            {
                if (xmlhttp2.readyState==4){// 4 = "loaded"
                    if (xmlhttp2.status==200){// 200 = "OK" 
                        jsonhouse=JSON.parse(xmlhttp2.responseText);
                        showhouse();
                        create_house1();
                    }else{
            ...
            function create_house1()
            {
                //清空场景 
                // scene.remove(mesh);
                // scene.remove(mesh_floor);
                scene.remove(initobj);
